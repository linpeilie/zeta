...
已物理删除旧 `AgentProvider` 兼容层。

- 删除 `agent_provider.dart`、`AgentProviderBundle.adapt`、全部 `_Legacy*`、旧 Factory adapter。
- 三个生产 Provider 只实现真实支持的中立端口；Grok/Claude 的 unsupported stub 已去掉。
- `DefaultAgentProviderFactory` 只保留 `createBundle`。
- 测试 Fake 改为实现端口，`fromFake` / `nativeTestBundle` 组装原生 Bundle。
- 架构测试锁定旧文件与 `adapt` 不存在。
- `format` / `analyze` / 相关测试已绿。

## S6 记录

Domain 不再列举厂商 capability 默认值。

- 删除 `AgentProviderCapabilities.codexAppServer/grokAcp/claudeCode` 与 `defaultsFor(kind)`。
- 新增 data 层 `AgentProviderStaticCapabilities`；settings / 三个 Provider / 测试 harness 从这里取静态默认值。
- `AgentProviderCapabilities` 仍是中立值对象：端口存在性是「操作是否存在」的真源；布尔位只作初始化前保守声明和握手后动态覆盖。`canForkThreadAtTurn` 仍留在值对象上（Codex 按 CLI 版本动态打开）；搬到 `AgentThreadBranchingPort` 会改端口契约，超出本步范围。
- 架构守卫：domain capabilities 文件不得再含 `defaultsFor`、厂商命名常量和 `AgentProviderKind.` 分支。
- 文档已改为 native Bundle：`AGENTS.md`、工程规范、设计文档、overview、developer_guide、glossary、CONTRIBUTING（含英文）。
- Pi 计划三份文件与 Claude 历史提案已追加 superseded 指针。
- ViewModel 目录测试改断言 `listModelsCalls`（旧 `refreshModels` 已随大接口删除）。
- `format` / `analyze` / 相关测试已绿。
