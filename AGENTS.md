...
- **Project Threads owner 边界**：同步命令、列表事实与 thread → project 索引只由 `ProjectThreadsSliceStore` 拥有；Shell/Widget/业务测试经 `ProjectThreadsOperations`，Runner 只执行 effect 和 I/O 调度。远端归属从 `StateOwner.threadFor` 读取摘要，不猜活跃 Provider；分页不得清掉窗口外显式映射，关闭后 ingress 不复活索引或通知选中项移除。当前 Deferred 与 presentation 镜像待 WP-3P 迁移。
