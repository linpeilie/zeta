...
- **环境开关不做布尔参数，做实现。** "接管原生窗口"是 `ZetaWindowHost` 的两个实现，"显示语言等不等持久化设置"是 `ZetaDisplayLanguageSource` 的两个实现——不是一路传下去的 `bool`。加平台调用时只改实现，调用点不必再补一次 `if`。平台 `WindowListener` 只允许出现在 app 窗口模块（`ZetaWindowSurfaceNotifier`）和关窗 hook；`MainApp` / `IdeHome` 禁止 mixin，读 `zetaWindowSurfaceProvider`。
