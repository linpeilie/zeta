...
- **application 与 domain 都不许 import Flutter**（目标架构 §12.5）：`@immutable` 走 `package:meta`，集合相等走 `zeta_foundation` 的 `zeta*Equals`，监听自己维护 listener 列表而不是 `ChangeNotifier`。Riverpod 是独立的 adapter 层，`Provider` / `Notifier` 只能出现在 presentation 或 `app` 组合层。守卫：`feature_layering_guard_test`（既有违规进燃尽清单，只减不增）。
