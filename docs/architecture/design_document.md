...
套餐时显示 Provider 名）、最短周期额度进度和今日 Token；没有额度窗口时不渲染套餐进度行。
冷加载 Skeleton 与摘要同结构，右上角保留开合按钮。展开态以折叠摘要为锚点向上弹出
...
横向胶囊画廊排布，卡片使用紧凑内边距与元信息层级的重置时刻；距当前 7 天内显示 `mm-dd HH:mm`，
超过 7 天只显示 `mm-dd`，Claude Code 五小时窗口显示为 `5h`。无额度窗口时隐藏整块套餐区。点击弹层外部或
...
增删或 Wide/Medium 断点切换时仍复用原 Element。左栏显隐、左栏宽度和
统计 Provider 选择投影进应用级 `IdeWorkbenchLayoutState`，随 `ide_session.json`
宽容恢复；统计展开态改由 Popover 承载，属于临时 UI，不再投影进会话，旧高度比例与旧展开
标记只为历史快照保留，不再参与布局。离开其他页面
