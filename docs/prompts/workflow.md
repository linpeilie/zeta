...
**产出物放哪：** 阶段 ①②③ 的产出默认写在 Issue 或 PR 描述里，**不入库**。本项目历史上删过 `plan/` 目录，不要再往仓库里堆临时方案文件。确实需要长期留档的设计（比如新 Provider 的适配方案）才进 `docs/architecture/` 或 `docs/protocols/`。
...
**产出物：** 问题陈述 · 验收标准清单 · 非目标 · 待决策项 · 复杂度档位
...
出方案，先不要写代码。用 codegraph explore 而不是逐个读文件来摸清现状。
...
**产出物：** 分层归属 · 契约变更清单 · 门禁合规论证 · 兼容性策略 · 测试矩阵 · 风险
...
做完这一步就停下来：跑 dart format . && flutter analyze && flutter test，
把结果告诉我，等我确认再进下一步。不要一口气做完所有步骤。
```

连续推进时用这个：

```text
按步骤表往下做，每完成一步跑一次 format + analyze + 相关测试。
...
而不是写在注释里。给我「问题 → 答案 → 用哪个测试固定」的对照表。
```
