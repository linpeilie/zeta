...
中文 ｜ [English](../../en/protocols/claude_code_stream_json_protocol.md)

最后更新：2026-08-19（自旧仓库 `bfd4241` 迁入，内容基线 2026-08-17）

本文记录 Zeta Claude Code Provider 的实际协议边界、已验证帧形状和升级门禁。
它是实现与维护时的事实基线；早期取舍和未落地设想见
[Claude Code Provider 接入适配文档（历史提案）](../history/claude_code_provider_adapter.md)。

> **迁移期读法**：本文描述的行为是 VGV 迁移的**功能等价目标**，实现位置改为
> `packages/claude_code_client/`（Data 层）。文中的"Provider"“data 层”对应新拓扑的
> `claude_code_client`；"application/共享层"对应 `agent_conversation_repository`。
> 端口签名见[包 API 契约](../architecture/package_api_contracts.md)。
...
| fixture | 迁移目标 `packages/claude_code_client/test/fixtures/`（旧路径 `test/src/features/agent/data/datasources/claude_code/fixtures/`） |
...
4. 跑 Provider mapper/identity/peer、live-history parity、权限、Plan、metadata、auth、额度、
   模型缓存与 compact 测试。
5. 跑共享层 purity guard，确认 `agent_conversation_repository` 的 Pipeline、Coalescing、
   timeline aggregate 与 `agent_provider_contracts` 端口无 Claude 专属改动。
