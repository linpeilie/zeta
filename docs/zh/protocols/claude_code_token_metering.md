...
中文 ｜ [English](../../en/protocols/claude_code_token_metering.md)

> 基于本地源码树：`/Users/linpeilie/Development/Workspace/OpenSource/claude-code`  
> 锚定 commit：`2ccc2162`（测绘时 HEAD）  
> 配套文档：[Claude Code Provider 接入适配（历史提案）](../history/claude_code_provider_adapter.md)、[Claude Code stream-json 协议基线](./claude_code_stream_json_protocol.md)  
> 约束基线：[迁移拓扑 §3 分层判定规则](../architecture/migration_topology.md)、[迁移任务清单 §1 全局约定](../architecture/migration_tasks.md)

> **迁移期读法**：本文写于旧仓库，文中的 G1–G8 是旧 `AGENTS.md` 的架构约束编号。
> 新仓库的等价约束见下表，正文中的引用按此对照阅读。
>
> | 旧编号 | 本文用到的含义 | 新仓库等价约束 |
> | --- | --- | --- |
> | G1 | 共享层不得按 Provider 分支 | [迁移拓扑 §3.2](../architecture/migration_topology.md)：Repository 不含 vendor 分支；门禁 §9.3 vendor client 互不依赖 |
> | G2 | 协议身份在 data 层消化 | [迁移拓扑 §3.1](../architecture/migration_topology.md)：Data 返回 typed model，不泄漏裸 JSON |
> | G4 | 能力缺失时不显示入口 | [迁移任务清单 步骤 33](../architecture/migration_tasks.md)：19 个 optional port 各有"入口不出现"测试 |
> | G6 | domain 保持中立，不引入厂商字段 | ADR-001：`agent_provider_contracts` 零 vendor 字段 |
> | G7 | 派生索引只保存白名单字段 | [迁移拓扑 §11](../architecture/migration_topology.md)：敏感凭据与 Provider 内容不进入日志 |
...
| # | 建议 | 落点 | 门禁 |
|---|---|---|---|
| R1 | stream-json `result.usage` / assistant usage → `AgentTokenUsage`：`input`、`output`、`cache_creation`→可映射 `cachedInputTokens` 或扩展字段、`cache_read` | `packages/claude_code_client/` 的 mapper | G2/G6：身份与字段在 Data 层消化 |
...
| R3 | 套餐五小时/周限额继续走[适配文档](../history/claude_code_provider_adapter.md) §4.11 REST，**不要**从 Spinner/粗估推 | `AgentUsageQuotaProvider` 端口 | 与 token 窗口占用语义隔离 |
| R4 | 不把 CC 的 `chars/4` 写进 `agent_conversation_repository` 的 pipeline / coalescing policy | 若做流式估，放 Presentation helper 或 `claude_code_client` 私有 helper | **G1** |
...
5. **测试**：用 Provider 无关的 delta fixture 测计数；禁止在 `agent_conversation_repository`
   的 coalescing 测试里依赖 Claude 类型。
...
块类型表可移植 §5.5；**image/document = 2000** 必须保留。实现放在 `agent_conversation_repository`
的纯函数 + 单测，输入为中立 timeline entry，而不是 Anthropic SDK 类型。
...
- **必须做**：本文 R1–R3 + [适配文档](../history/claude_code_provider_adapter.md) §4.4 `AgentTokenUsageEvent`、§4.11 套餐。  
