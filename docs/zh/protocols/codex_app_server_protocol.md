...
中文 ｜ [English](../../en/protocols/codex_app_server_protocol.md)

最后更新：2026-08-19（自旧仓库 `bfd4241` 迁入，内容基线 2026-08-12）

> **迁移期读法**：适配层实现位置由旧仓库 `lib/src/features/agent/data/datasources/app_server/`
> 改为 `packages/codex_app_server_client/`。schema 快照与生成脚本按
> [迁移清单](../architecture/migration_manifest.md)迁入新仓库同名路径
> `third_party/codex_app_server_schema/` 与 `tool/`。
...
| 适配计划 | `plan/codex_app_server_adaptation_plan.md`（旧仓库已随 `plan/` 目录移除，仅存于旧仓库 Git 历史；不迁入本仓库） |
...
4. 按 diff 更新 `packages/codex_app_server_client/` 适配层与测试。
5. 更新本文件的 pinned 版本说明。
...
- UI / Bloc / Repository 只消费 `agent_provider_contracts` 的中立 `AgentEvent` 等模型，
  不直接读 schema JSON。
...
Zeta 根据成功的 Plan turn 和已归一化的 Plan 内容创建本地
`AgentPlanExecutionRequest`；该请求不进入 JSON-RPC、JSONL 历史或 Provider pending
registry。迁移后该交接的判定归 `AgentConversationBloc`（见
[会话状态设计 §6.4](../architecture/agent_conversation_state_design.md)），Plan 内容归一化仍在
...
与 command-only 四条互不冒充的路径。`CodexFileChangeTracker` 在 `codex_app_server_client`
内完成 identity、累计快照和优先级；`agent_conversation_repository` 与 UI 不读取 App Server
或 session JSONL raw 字段。
...
后到的 tool snapshot 会先产生 empty turn snapshot 清除 fallback。Repository/UI 不按路径跨 owner
