...
/// 将类型化 UI 请求合并到 frame 边界的 application 调度器。
///
...
///
/// [frameScheduler] 必须由组合层注入：生产用 presentation 的
/// `SchedulerBindingAgentFrameScheduler`，测试用 fake。application 不 import Flutter。
final class AgentUiUpdateScheduler implements AgentUiUpdatePort {
  AgentUiUpdateScheduler(
    this._onPublish, {
    required AgentFrameScheduler frameScheduler,
  ...
  }) : _frameScheduler = frameScheduler, // ignore: prefer_initializing_formals
       _metrics = metrics,
  ...
  /// 丢弃 pending 请求；已经交给 frame scheduler 的 callback 到达时只会安全退出。
  void dispose() {
    if (_disposed) {
      return;
    }
