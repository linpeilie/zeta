...
enum ClaudeCodeTerminalSource { resultFrame, interrupt, cancel, providerError }
...
final class ClaudeCodeStreamIdentity {
  ClaudeCodeStreamIdentity({this.maxRecentRawEvents = 512})
  ...
  _ClaudeCodeTurnState? _resolveState({
    ...
    if (active != null &&
        (runningTurnId == null || runningTurnId == active.turnId)) {
      return active;
    }
    if (recordMissing) {
      _missingTurnScopeDropped += 1;
  ...
  String _createEntryId({
    ...
    final base =
        'cc:${state.sessionId.hashCode.toUnsigned(32).toRadixString(16)}:'
        '${state.turnId.hashCode.toUnsigned(32).toRadixString(16)}:'
