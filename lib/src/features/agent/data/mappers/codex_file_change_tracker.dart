...
List<_CodexUnifiedDiffChunk>? _codexUnifiedDiffChunks(String diff) {
  final headers = RegExp(r'^diff --git ', multiLine: true).allMatches(diff);
...
String _codexChangeId({
  ...
  final destinationKey = destinationPath?.replaceAll('\\', '/');
...
bool _codexSameEvidence(
  ...
  (
    ...
    AgentTextReplacementEvidence(
      oldText: final otherOldText,
      newText: final otherNewText,
      replaceAll: final otherReplaceAll,
    ),
  ) =>
    oldText == otherOldText &&
        newText == otherNewText &&
