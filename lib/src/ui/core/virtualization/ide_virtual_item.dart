...
/// [IdeExtentIndex] 做前缀和与 offset 查找。
library;
...
final class IdeVirtualItemDescriptor {
  ...
  /// 捕获，release 下由 [IdeExtentIndex.synchronize] 安全 clamp。
...
///
/// 记录由 [IdeExtentIndex] 拥有；外部调用方应只读，不要绕过索引改写字段。
final class IdeExtentRecord {
  ...
  /// 由 [IdeExtentIndex.synchronize] 更新 kind 与 revision。
  ...
  /// 直接写入前缀和高度；仅由索引在估算路径中使用。
  void setEffectiveExtent(double value) {
    effectiveExtent = value;
  }
