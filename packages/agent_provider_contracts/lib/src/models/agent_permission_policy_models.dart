// 中立权限策略领域模型与端口。
// The resolved constructor keeps its named parameter non-nullable while the
// shared field remains nullable for the provider-fallback constructor.
// ignore_for_file: prefer_initializing_formals

import 'package:agent_provider_contracts/src/bundle/agent_provider_bundle.dart'
    show AgentProviderBundle;
...
/// Provider 暴露的权限选项目录。
@immutable
final class AgentPermissionCatalog {
  /// 创建不可变目录快照。
  ///
...
@immutable
final class AgentPermissionSelection {
  /// 创建仅含 optionId 的选择。
...
/// [AgentPermissionRequestSnapshot.providerFallback] 不携带 selection，
/// 由 data adapter 兼容现有默认逻辑。
@immutable
final class AgentPermissionRequestSnapshot {
  /// 创建已经由 application 解析完成的请求快照。
  // The field is nullable only for the separate providerFallback constructor.
  const AgentPermissionRequestSnapshot.resolved({
    ...
    required this.source,
       ...
       assert(
         ...
         'A resolved snapshot cannot use the provider fallback source.',
       );
...
/// 权限选择生效范围。
enum AgentPermissionWarningCode { appliesNextSession, downgradedByRuntime }
...
@immutable
final class AgentPermissionApplyResult {
  /// 创建应用结果。
  ...
  /// Typed warning mapped to localized copy by Presentation.
  final AgentPermissionWarningCode? warningCode;
  ...
  @override
  int get hashCode => Object.hash(normalizedSelection, scope, warningCode);
}
