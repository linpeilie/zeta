...
final class AgentThreadHistorySnapshot {
...
final class AgentHistoryReasoningEffort {
  ...
  const AgentHistoryReasoningEffort.explicit(String explicitValue)
    : value = explicitValue,
...
/// 一个 turn 的历史聚合结果。
final class AgentHistoryTurn {
  AgentHistoryTurn({
    required this.id,
    List<AgentHistoryEntry> entries = const <AgentHistoryEntry>[],
...
final class AgentTurnModelConfig {
...
/// 历史消息条目。
final class AgentHistoryMessageEntry extends AgentHistoryEntry {
  AgentHistoryMessageEntry({
    ...
    super.raw,
  }) : localImagePaths = immutableList(localImagePaths);
...
/// 历史工具调用条目。
final class AgentHistoryToolEntry extends AgentHistoryEntry {
...
final class AgentUserInputQaPair {
  AgentUserInputQaPair({
    required this.questionId,
    required this.question,
    this.header,
    List<String> options = const <String>[],
    List<AgentUserInputOption> optionItems = const <AgentUserInputOption>[],
    List<String> answers = const <String>[],
    this.allowMultiple = false,
    this.isOther = false,
    this.isSecret = false,
  }) : options = immutableList(options),
       optionItems = immutableList(optionItems),
       answers = immutableList(answers);
...
/// 非消息/非工具的历史事件条目。
final class AgentHistoryEventEntry extends AgentHistoryEntry {
