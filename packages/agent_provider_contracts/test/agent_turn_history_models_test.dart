...
void main() {
  ...
  group('AgentTurnModelConfig', () {
    test('uses only Provider-normalized model config', () {
      final turn = AgentHistoryTurn(
        id: 'turn-1',
        modelId: 'gpt-5.5',
        reasoningEffort: const AgentHistoryReasoningEffort.explicit('high'),
    ...
    test('ignores every raw model config alias', () {
      final turn = AgentHistoryTurn(
        ...
        reasoningEffort: const AgentHistoryReasoningEffort.explicit('xhigh'),
        explicitFast: false,
        raw: <String, Object?>{
          'turnContext': <String, Object?>{
            'model': 'raw-model',
    ...
    test('keeps unknown and explicit provider default distinct', () {
      final unknown = AgentHistoryTurn(id: 'turn-unknown', modelId: 'gpt-5.5');
      final providerDefault = AgentHistoryTurn(
        id: 'turn-default',
        modelId: 'gpt-5.5',
        reasoningEffort: const AgentHistoryReasoningEffort.providerDefault(),
      );
    ...
    test('returns the latest typed mode while skipping missing turns', () {
      ...
      final snapshot = AgentThreadHistorySnapshot(
        ...
        turns: <AgentHistoryTurn>[
          ...
          AgentHistoryTurn(id: 'turn-missing'),
          AgentHistoryTurn(id: 'turn-future', collaborationMode: unknownMode),
          AgentHistoryTurn(id: 'turn-latest-missing'),
        ],
        currentTurn: AgentHistoryTurn(id: 'turn-latest-missing'),
      );
