...
import 'package:app_ui/src/components/ide_extent_index.dart'
    show IdeExtentIndex;
import 'package:app_ui/src/components/ide_virtual_item.dart';
import 'package:app_ui/src/components/ide_virtual_list_controller.dart';
...
class RenderIdeAnchoredDynamicSliverList extends RenderSliverMultiBoxAdaptor {
  ...
  void _assertSupportedConfiguration(SliverConstraints constraints) {
    // Debug-only diagnostics for a constructor-level integration contract.
    ...
    assert(() {
      ...
      return true;
    }(), 'Only vertical forward-growing slivers are supported');
    // coverage:ignore-end
  }
  ...
  void performLayout() {
    ...
    // A non-empty index can only be produced by synchronize, which records
    // the epoch atomically with its records.
    final epoch = extentIndex.epoch!;

    final scrollOffset = constraints.scrollOffset + constraints.cacheOrigin;
    assert(scrollOffset >= 0.0, 'scrollOffset must not be negative');
    final remainingExtent = constraints.remainingCacheExtent;
    assert(remainingExtent >= 0.0, 'remainingExtent must not be negative');
    ...
    assert(firstChild != null, 'A non-empty index must lay out a first child');
    assert(
      debugAssertChildListIsNonEmptyAndContiguous(),
      'Laid-out sliver children must be contiguous',
    );
  ...
  void _measureAndPlaceChild({
    ...
    final parentIndex = parentData.index;
    parentData.layoutOffset = extentIndex.offsetOf(index);
    assert(parentIndex == index, 'Child parent-data index must match its slot');
  }
