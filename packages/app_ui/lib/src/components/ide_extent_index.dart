...
final class IdeExtentIndex {
  ...
  int indexAtOffset(double scrollOffset) {
    ...
    var normalizedOffset = scrollOffset;
    if (!normalizedOffset.isFinite) {
      assert(() {
        debugPrint(
          'IdeExtentIndex.indexAtOffset 收到非有限 offset=$normalizedOffset， '
          '将按 0 处理。',
        );
        return true;
      }(), 'Non-finite offsets are normalized to zero');
      normalizedOffset = 0;
    }
    if (normalizedOffset >= totalExtent) {
    ...
    return _tree.lowerBoundExclusiveEnd(normalizedOffset);
  }
  ...
  void synchronize(
    ...
    final nextRecords = List<IdeExtentRecord>.generate(descriptors.length, (
      ...
      if (previous.hasMeasuredExtent) {
        ...
        previous.effectiveExtent = estimated;
      }
      return previous;
    }, growable: false);
  ...
  void _assertUniqueIds(List<IdeVirtualItemDescriptor> descriptors) {
    assert(() {
      ...
      return true;
    }(), 'Virtual item IDs must be unique');
  }
