...
/// Bundled monospace family used by code-oriented typography tokens.
const String bundledCodeFontFamily = 'JetBrainsMono';

/// Bundled interface family used by the Zeta desktop theme.
const String bundledUiFontFamily = 'Geist';

/// Default interface font size in logical pixels.
const double defaultUiFontSize = 12;

/// Default code font size in logical pixels.
const double defaultCodeFontSize = 12;

/// IDE 设计系统排版 token。
///
/// 通过 [of] 从 Material theme 读取当前颜色与字体/字号，按比例缩放生成
...
class AppTypography extends ThemeExtension<AppTypography> {
  /// Creates a complete semantic typography table.
  const AppTypography({
  ...
  /// 字重顶到最粗、颜色停在 [AppColors.textSecondary] 来和行内容拉开分工——
  ...
  /// 从当前上下文解析语义排版。
  ///
  /// 字体与字号来自当前 theme（由外观设置注入）；颜色来自
  /// [AppColors.of]。可选 [codeFontFamily] 仅覆盖代码字体。
  // Context lookup intentionally remains a named helper rather than a factory.
  // ignore: prefer_constructors_over_static_methods
  static AppTypography of(BuildContext context, {String? codeFontFamily}) {
    final configured = Theme.of(context).extension<AppTypography>();
    if (configured != null &&
        (codeFontFamily == null || codeFontFamily.isEmpty)) {
      return configured;
    }
  ...
  // The migrated semantic table remains close to its resolver inputs.
  ...
  AppTypography copyWith({
    ...
    return AppTypography(
      ...
      proseBody: proseBody ?? this.proseBody,
      meta: meta ?? this.meta,
