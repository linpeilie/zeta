  ...
  group('19.1 IdeExtentIndex', () {
    ...
    test('5. stable ID synchronize 复用测量', () {
      final index = IdeExtentIndex()
        ...
        ..updateMeasuredExtent(index: 0, measuredExtent: 120, epoch: epochA);

      // Keep mutation phases separate so the assertions mirror runtime syncs.
    ...
    test('6. prepend 保留后续 ID 的测量并重建 offset', () {
      final index = IdeExtentIndex()
        ...
        ..updateMeasuredExtent(index: 0, measuredExtent: 80, epoch: epochA)
        ..updateMeasuredExtent(index: 1, measuredExtent: 90, epoch: epochA);
    ...
    test('7. remove 后映射与总高度正确', () {
      final index = IdeExtentIndex()
        ...
        ..updateMeasuredExtent(index: 1, measuredExtent: 50, epoch: epochA);
    ...
    test('8. reorder 按新顺序重建 prefix 并复用测量', () {
      final index = IdeExtentIndex()
        ...
        ..updateMeasuredExtent(index: 0, measuredExtent: 15, epoch: epochA)
        ..updateMeasuredExtent(index: 2, measuredExtent: 45, epoch: epochA);
    ...
    test('11. epoch 变化保留旧值但标记 stale', () {
      final index = IdeExtentIndex()
        ...
        ..updateMeasuredExtent(index: 0, measuredExtent: 200, epoch: epochA)
        ..updateMeasuredExtent(index: 1, measuredExtent: 80, epoch: epochA);
...
final class _NaiveExtentArray {
  ...
  int indexAtOffset(double scrollOffset) {
    ...
    final normalizedOffset = scrollOffset.isFinite && scrollOffset >= 0
        ? scrollOffset
        : 0.0;
