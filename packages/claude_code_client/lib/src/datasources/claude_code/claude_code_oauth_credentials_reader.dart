...
final class ClaudeCodeOAuthCredentials {
  /// Creates a [ClaudeCodeOAuthCredentials].
...
typedef ClaudeCodeCredentialsFileSource = Future<String?> Function(String path);

/// 从本机文件系统读取 Claude Code 凭据。
final class FileClaudeCodeCredentialsFileSource {
  /// Creates a [FileClaudeCodeCredentialsFileSource].
  const FileClaudeCodeCredentialsFileSource();

  /// Reads the credential file at [path] when it exists.
...
final class ClaudeCodeOAuthCredentialsReader {
  /// Creates a [ClaudeCodeOAuthCredentialsReader].
  ClaudeCodeOAuthCredentialsReader({
    ...
    DateTime Function()? clock,
       ...
       _fileSource =
           fileSource ?? const FileClaudeCodeCredentialsFileSource().read,
       _secureSource =
           secureSource ??
           ClaudeCodeMacOsKeychainSource(environment: environment).read,
  ...
  Future<ClaudeCodeOAuthCredentials?> read() async {
    if (_isMacOS) {
      try {
        final source = await _secureSource();
    ...
    try {
      final source = await _fileSource(path);
