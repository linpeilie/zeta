...
enum ClaudeCodeSessionToolDecision {
  /// Always allow the tool within the current session.
  allow,

  /// Always deny the tool within the current session.
  deny,
}
...
final class FileClaudeCodeSessionDecisionStore
    implements ClaudeCodeSessionDecisionStore {
  /// Creates a [FileClaudeCodeSessionDecisionStore].
  FileClaudeCodeSessionDecisionStore({required File file, AppLogger? logger})
    : _file = AtomicTextFile(file),
      _log = logger ?? loggerFor('zeta.agent.claude_code.permission_policy');
...
final class ClaudeCodePermissionPolicyAdapter
  ...
  /// Creates a [ClaudeCodePermissionPolicyAdapter].
