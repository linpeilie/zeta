...
void main() {
  ...
  test(
    'duplicate Write evidence and unsupported updates reuse the snapshot',
    () {
      final first = _record(
        tracker,
        toolName: 'Write',
        kind: AgentToolKind.edit,
        input: const <String, Object?>{
          'file_path': '/workspace/reused.txt',
          'content': 'same',
        },
      );
      final duplicate = _record(
      ...
      final unsupported = _record(
        tracker,
        toolName: 'Bash',
        kind: AgentToolKind.execute,
        input: const <String, Object?>{'command': 'true'},
      );

      expect(duplicate.fileChanges, same(first.fileChanges));
      expect(unsupported.fileChanges, same(first.fileChanges));
    },
  );
