...
void main() {
  group('ClaudeCodePermissionPolicyAdapter', () {
    ...
    test('file cache tolerates damaged and unknown-version JSON', () async {
      ...
      await file.writeAsString(
        jsonEncode(<String, Object?>{
          ...
          'decisions': <String, Object?>{'not': 'a list'},
        }),
      );
      expect(await store.load(), isEmpty);
    ...
    test(
      'stale question cleanup remains in memory when persistence fails',
      () async {
        final store = _FailingCleanupDecisionStore();
        final adapter = ClaudeCodePermissionPolicyAdapter(
          applyPermissionMode: (_) async =>
              AgentPermissionApplyScope.nextSession,
          sessionDecisionStoreFactory: (_) => store,
...
final class _FailingCleanupDecisionStore
  ...
  Future<Map<String, ClaudeCodeSessionToolDecision>> load() async =>
      <String, ClaudeCodeSessionToolDecision>{
        'AskUserQuestion': ClaudeCodeSessionToolDecision.deny,
