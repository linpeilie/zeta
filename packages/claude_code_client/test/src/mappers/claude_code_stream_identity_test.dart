...
void main() {
  ...
  group('ClaudeCodeStreamIdentity §3.B rows 1–6', () {
    ...
    test(
      'covers explicit invalidation, active fallback, and duplicate terminal',
      () {
        final identity = ClaudeCodeStreamIdentity()
          ..beginTurn(
            runtimeScope: scope1,
        ...
        final first = identity.completeTurn(
          runtimeScope: scope1,
          sessionId: 'session-edge',
          runningTurnId: 'turn-edge',
          status: AgentHistoryTurnStatus.completed,
          source: ClaudeCodeTerminalSource.resultFrame,
          eventId: 'terminal-edge',
        );
        ...
        expect(duplicate.disposition, ClaudeCodeTerminalDisposition.duplicate);
        expect(
          identity.resolveReasoning(
            runtimeScope: scope1,
            sessionId: 'session-edge',
            runningTurnId: 'turn-edge',
            sourceItemId: 'reasoning-edge',
        ...
        identity
          ...
          ..invalidateTurn(
            runtimeScope: scope1,
            sessionId: 'session-invalidated',
            runningTurnId: 'turn-invalidated',
            reason: ClaudeCodeIdentityInvalidationReason.cancel,
          );
        expect(
          identity.resolveMetadata(
            runtimeScope: scope1,
    ...
    test('collision suffix advances until an unused identity is found', () {
      final identity = ClaudeCodeStreamIdentity(identityToken: (_) => 'same');
      addTearDown(identity.dispose);
      final entryIds = <String>[];
      for (final sessionId in <String>['session-a', 'session-b', 'session-c']) {
        identity.beginTurn(
          runtimeScope: scope1,
          sessionId: sessionId,
          turnId: 'turn',
      ...
      expect(entryIds.last, endsWith(':g2'));
    });
  });
}
