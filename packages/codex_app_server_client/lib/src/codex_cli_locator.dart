...
typedef CodexCliFileExists = Future<bool> Function(String path);

/// Locates Codex from persisted configuration, PATH, and known install roots.
final class CodexCliLocator {
  ...
  final CodexCliFileExists? fileExists;
  ...
  /// Validates a user-selected Codex path.
  ...
  Iterable<String> _commonCandidates(Map<String, String> values) sync* {
    if (_isWindows) {
      ...
      if (localAppData != null && localAppData.isNotEmpty) {
        ...
        yield _join(localAppData, 'Programs', 'codex', 'codex.exe');
        yield _join(localAppData, 'npm', 'codex.cmd');
      }
      final appData = values['APPDATA'];
      if (appData != null && appData.isNotEmpty) {
        yield _join(appData, 'npm', 'codex.cmd');
      }
      return;
    }
    final home = values['HOME'];
    if (home != null && home.isNotEmpty) {
      yield _join(home, '.local', 'bin', 'codex');
    }
...
/// Whether [path] has an exact Codex executable basename.
