...
import 'package:codex_app_server_client/src/codex_static_capabilities.dart';
...
part '../../history/codex_jsonl_history_parser.dart';
part '../../history/codex_thread_history_reader.dart';
part '../../testing/codex_protocol_test_harness.dart';
...
class CodexAppServerAgentProvider
  ...
  CodexAppServerAgentProvider({
    ...
    AppLogger? logger,
       ...
       _log = logger ?? loggerFor('zeta.agent.codex_app_server') {
    _peer = ProviderRuntimeJsonRpcPeer(
      peer ?? _defaultPeerFactory(config, logger: _log),
  ...
  AgentProviderCapabilities _capabilities = CodexStaticCapabilities.value;
  ...
  StreamSubscription<TransportException>? _protocolErrorSubscription;
    ...
    try {
      ...
      _emitStatus(
        AgentProviderStatus(
          ...
          code: AgentProviderStatusCode.failure,
          diagnosticDetails: error.runtimeType.toString(),
        ),
      );
      _events.add(
        AgentErrorEvent(
  ...
  /// Refetches the Codex model catalog, bypassing the in-memory cache.
  ...
  /// Codex-specific permission catalog and selection adapter.
...
JsonRpcPeer _defaultPeerFactory(
  ...
  return JsonRpcStdioTransport(
    ...
    processStarter: codexProcessStarter(config, logger: logger),
    logger: logger,
  );
}
