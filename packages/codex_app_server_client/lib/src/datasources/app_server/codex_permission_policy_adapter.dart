...
typedef CodexPermissionRpcSender = Future<Object?> Function(
  String method, {
...
final class CodexPermissionPolicyAdapter implements AgentPermissionPolicyPort {
  ...
  CodexPermissionPolicyAdapter({
    ...
    AppLogger? logger,
  }) : _log = logger ?? loggerFor('zeta.agent.codex_permission_policy');

  /// Ensures the app-server handshake is complete before catalog requests.
  final Future<void> Function() ensureInitialized;

  /// Sends a raw JSON-RPC request to the active app-server connection.
