// Test setup intentionally mutates fake peers in sequential statements.
...
void main() {
  group('CodexAppServerAgentProvider', () {
    ...
    test('does not log realtime notifications and server requests', () async {
      ...
      expect(
        fineMessages.where(
          (message) => message.toString().contains('Realtime server request'),
        ),
        isEmpty,
      );
    ...
    test('logs every unmatched notification and counts occurrences', () async {
      ...
      final unmatchedFineMessages = records
          .where(
            (record) =>
                ...
                record.message.toString().contains(
      ...
      final skillsChangedMessages = records
          .where(
            (record) =>
                record.level == Level.trace &&
                record.message.toString().contains('skills/changed received'),
          )
          .map((record) => record.message)
          .toList();
      ...
      () async {
        ...
        expect(
          codexFineMessages.where(
            (message) =>
                message.toString().contains('mcpServer/startupStatus/updated'),
          ),
          hasLength(1),
        );
        expect(
    ...
    test(
      ...
      () async {
        ...
        await provider.respondToQuestion(
          AgentQuestionResponse(requestId: 'input-resolved'),
        );
    ...
    test('coalesces concurrent provider initialization', () async {
      ...
      final firstList = provider.listThreads(
        query: AgentThreadListQuery(projectPath: '/repo', limit: 5),
      );
    ...
    test('encodes localImage inputs in turn/start', () async {
      ...
      await provider.sendMessage(
        ...
        inputs: <AgentUserInput>[
          ...
          const AgentUserInput.localImage(path: r'D:\tmp\shot.png'),
        ],
        context: const AgentContext(projectPath: '/repo'),
      );
    ...
    test(
      ...
      () async {
        ...
        addTearDown(provider.dispose);
        final threadA = AgentSession(
          id: 'thread-a',
          providerId: defaultAgentProviderId,
        );
        final threadB = AgentSession(
          id: 'thread-b',
          providerId: defaultAgentProviderId,
        );
    ...
    test('dispose closes pending approvals and rejects new RPC', () async {
      ...
      await expectLater(
        provider.listThreads(
          query: AgentThreadListQuery(projectPath: '/repo', limit: 20),
        ),
    ...
    test('maps autoApprovalReview notifications', () async {
      ...
      await provider.approveDeniedAction(
    ...
    test(
      'covers provider failure, lifecycle, and transport recovery paths',
      () async {
        final defaultProvider = CodexAppServerAgentProvider(
          config: AgentProviderConfig.defaultCodex,
        );
        expect(defaultProvider.runtimeScope, isNull);
        await defaultProvider.dispose();

        for (final error in <Object>[
          const ProcessException('codex', <String>[], 'missing', 2),
          StateError('handshake failed'),
        ]) {
          final failed = CodexAppServerAgentProvider(
            config: AgentProviderConfig.defaultCodex,
            peer: _FakeJsonRpcPeer(startError: error),
          );
          await expectLater(failed.initialize(), throwsA(same(error)));
          await failed.dispose();
        }

        final peer = _FakeJsonRpcPeer(
          requestErrors: <String, Object>{
            'thread/unsubscribe': StateError('unsubscribe failed'),
          },
        );
        final provider = CodexAppServerAgentProvider(
          config: AgentProviderConfig.defaultCodex,
        ...
        await provider.initialize();
        expect(provider.runtimeScope, isNotNull);
        await provider.permissionPolicy.listPermissionOptions();
        await provider.listModels(forceRefresh: true);
        await provider.unsubscribeThread('thread-with-error');

        final inactive = AgentSession(id: 'inactive', providerId: 'codex');
        await expectLater(
          provider.steerTurn(
            session: inactive,
            expectedTurnId: 'turn',
        ...
        peer.emitProtocolError(
          const TransportMalformedFrame(message: 'malformed', payloadLength: 1),
        );
        peer.emitNotification('serverRequest/resolved', <String, Object?>{
          'requestId': 'unknown',
          'threadId': session.id,
        ...
        peer.emitNotification(
          ...
          <String, Object?>{
            ...
            'turnId': turn.id,
            'reviewId': 'review',
            'review': <String, Object?>{'status': 'approved'},
          },
        );
        peer.emitNotification(
          'item/autoApprovalReview/completed',
          <String, Object?>{
            ...
            'review': <String, Object?>{'status': 'denied'},
          },
        );
        peer.emitNotification('thread/tokenUsage/updated', <String, Object?>{
          'threadId': session.id,
        ...
        peer.emitNotification('turn/completed', <String, Object?>{
          'threadId': session.id,
          'turn': <String, Object?>{'id': turn.id, 'status': 'completed'},
        });
        peer.emitNotification('thread/closed', <String, Object?>{
          'threadId': session.id,
        });
        ...
        expect(peer.requestMethods, contains('turn/interrupt'));
        expect(events.whereType<AgentErrorEvent>(), isNotEmpty);
      },
    );

    test(
      'resolves pending questions and logs failed server responses',
      () async {
        final peer = _FakeJsonRpcPeer(failResponses: true);
        final provider = CodexAppServerAgentProvider(
          config: AgentProviderConfig.defaultCodex,
          peer: peer,
        );
    ...
    test('maps every Codex usage limit reason code', () async {
      for (final reason in <String>[
        ...
        final peer = _FakeJsonRpcPeer(
          accountRateLimitsResponse: <String, Object?>{
            'rateLimits': <String, Object?>{
              ...
              'primary': <String, Object?>{'usedPercent': 100},
            },
          },
        );
        final provider = CodexAppServerAgentProvider(
        ...
        final quota = await provider.readUsageQuota();
        expect(quota?.reachedReasonCode, isNotNull);
        await provider.dispose();
      }
    });

    test('keeps local history when remote enrichment fails', () async {
      final file = await _writeJsonlFile(<Object?>[
        <String, Object?>{
          'type': 'session_meta',
          'payload': <String, Object?>{'id': 'local-thread'},
        },
        <String, Object?>{
          'type': 'event_msg',
          'payload': <String, Object?>{
      ...
      final provider = CodexAppServerAgentProvider(
        ...
        peer: _FakeJsonRpcPeer(
          requestErrors: <String, Object>{
            'thread/read': StateError('remote unavailable'),
          },
        ),
      );
      final history = await provider.readThreadHistory(
      ...
      expect(history.turns.single.id, 'local-turn');
      await provider.dispose();
    });
  });
}
...
class _FakeJsonRpcPeer implements JsonRpcPeer {
  _FakeJsonRpcPeer({
    ...
    this.startError,
    this.requestErrors = const <String, Object>{},
    this.failResponses = false,
  ...
  final Map<String, Object> requestErrors;
  final bool failResponses;
  ...
  Future<Object?> sendRequest(
    ...
    if (requestErrors[method] case final error?) {
      Error.throwWithStackTrace(error, StackTrace.current);
    }
  ...
  Future<void> sendResponse(
    ...
    if (failResponses) {
      throw StateError('response failed');
    }
    if (error != null) {
