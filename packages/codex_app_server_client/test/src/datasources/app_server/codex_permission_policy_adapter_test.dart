...
import 'package:codex_app_server_client/src/datasources/app_server/codex_permission_policy_adapter.dart';
import 'package:codex_app_server_client/src/mappers/codex_permission_policy_codec.dart';
import 'package:json_rpc_transport/json_rpc_transport.dart';
import 'package:test/test.dart';

void main() {
  group('CodexPermissionPolicyCodec utility branches', () {
    test('normalizes labels, policies, and sandbox wire values', () {
      expect(
        CodexPermissionPolicyCodec.displayLabelForOptionId(' custom '),
        'custom',
      );
      expect(CodexPermissionPolicyCodec.displayLabelForOptionId(''), '');
      expect(
        CodexPermissionPolicyCodec.normalizePersistedApprovalPolicy(null),
        isNull,
      );
      expect(
        CodexPermissionPolicyCodec.normalizePersistedApprovalPolicy(
          'on-failure',
        ),
        'on-request',
      );
      expect(
        CodexPermissionPolicyCodec.toTurnSandboxPolicy('readOnly'),
        <String, Object?>{'type': 'readOnly'},
      );
      expect(
        CodexPermissionPolicyCodec.toThreadSandboxMode('readOnly'),
        'read-only',
      );
      expect(
        CodexPermissionPolicyCodec.toThreadSandboxMode('dangerFullAccess'),
        'danger-full-access',
      );
      expect(
        CodexPermissionPolicyCodec.toThreadSandboxMode('workspaceWrite'),
        'workspace-write',
      );
    });

    test('covers snapshot selection, copying, and equality semantics', () {
      const policiesOnly = CodexPermissionRuntimeSnapshot(
        approvalPolicy: 'never',
        sandboxPolicy: 'dangerFullAccess',
      );
      expect(policiesOnly.selectedOptionId, ':danger-full-access');
      expect(policiesOnly.displayLabel, 'Full access');

      const unknownPolicies = CodexPermissionRuntimeSnapshot(
        approvalPolicy: 'custom',
        sandboxPolicy: 'custom',
      );
      expect(unknownPolicies.selectedOptionId, isNull);
      expect(unknownPolicies.displayLabel, 'Workspace write');

      final copied = policiesOnly.copyWith(
        optionId: 'team-safe',
        approvalPolicy: 'on-request',
        sandboxPolicy: 'workspaceWrite',
        permissionProfileId: 'team-safe',
      );
      expect(copied.selectedOptionId, 'team-safe');
      expect(copied.displayLabel, 'team-safe');
      expect(
        copied.copyWith(
          clearOptionId: true,
          clearPermissionProfileId: true,
        ),
        const CodexPermissionRuntimeSnapshot(),
      );
      expect(copied, copied);
      expect(copied, isNot(policiesOnly));
      ...
      expect(
        CodexPermissionPolicyCodec.protocolPermissionProfileId(
          const CodexPermissionRuntimeSnapshot(optionId: ':workspace'),
        ),
        ':workspace',
      );
    });

    test('encodes policy-only snapshots and applies neutral selections', () {
      final applied = CodexPermissionPolicyCodec.applySelection(
        const AgentPermissionSelection(optionId: ' team-safe '),
      );
      expect(applied.permissionProfileId, 'team-safe');

      const fallback = CodexPermissionRuntimeSnapshot(
        approvalPolicy: 'custom',
        sandboxPolicy: 'customSandbox',
      );
      expect(
        CodexPermissionPolicyCodec.encodeThreadPermissionFields(fallback),
        <String, Object?>{
          'approvalPolicy': 'on-request',
      ...
      expect(
        CodexPermissionPolicyCodec.encodeTurnPermissionFields(fallback),
        <String, Object?>{
          'approvalPolicy': 'on-request',
          'sandboxPolicy': <String, Object?>{'type': 'customSandbox'},
        },
      );
    });
  });
  ...
  group('CodexPermissionPolicyAdapter', () {
    test('lists options across cursor pages', () async {
      ...
      final adapter = CodexPermissionPolicyAdapter(
        ...
        sendRequest: (method, {params = const {}}) async {
    ...
    test(
      'falls back when the experimental API is explicitly disabled',
      () async {
        final adapter = CodexPermissionPolicyAdapter(
          ensureInitialized: () async {},
          sendRequest: (method, {params = const {}}) async {
            throw const JsonRpcException(
              JsonRpcError(
                code: -32000,
                message: 'Experimental API is disabled',
              ),
            );
          },
        );
