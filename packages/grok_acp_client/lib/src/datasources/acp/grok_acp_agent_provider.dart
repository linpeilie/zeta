...
class GrokAcpAgentProvider
  ...
  GrokAcpAgentProvider({
    ...
    AppLogger? logger,
  }) : _log = logger ?? loggerFor('zeta.agent.grok_acp'),
  ...
  @override
  AgentProviderCapabilities get capabilities => GrokStaticCapabilities.value
  ...
  Future<AgentModelList> refreshModels({
    ...
    final describedList = fromCli.models.isNotEmpty
        ? fromCli
        : (_modelList ?? fromCli);
    _log.i(
  ...
  Future<AgentTurn> sendMessage({
    ...
    // initialize() 成功后 ProviderRuntimeJsonRpcPeer 必然已建立 runtime scope。
  ...
  Future<void> respondToPermission(AgentPermissionDecision decision) async {
    ...
    // 只有携带非空 options 的请求才会进入 pending 集合。
  ...
  Future<void> respondToPlanApproval(AgentPlanApprovalDecision decision) async {
    ...
    if (pending == null) {
      _log.w(
        'Ignoring response for unknown Grok plan approval '
        '${decision.requestId}',
      );
      return;
    }
  ...
  void _listenToPeer() {
    ...
    _serverRequestSubscription ??= _peer.serverRequests.listen((request) {
      // ProviderRuntimeJsonRpcPeer scopes the request; _handleServerRequest
      // contains all protocol failures and always attempts a response.
      unawaited(_peer.handleServerRequest(request, _handleServerRequest));
    });
    ...
    _protocolErrorSubscription ??= _peer.protocolErrors.listen((error) {
      if (error is TransportMalformedFrame &&
          error.message == 'Response for unknown request id') {
  ...
  void _handleNotification(JsonRpcNotification notification) {
    ...
    // ProviderRuntimeJsonRpcPeer attaches the active scope before forwarding.
  ...
  Future<void> _handleQuestionRequest(JsonRpcRequest request) async {
    ...
    _log.i(
      ...
      '(id=${mapped.pending.id}, '
      'questions=${mapped.event.request.questions.length})',
    );
