...
final AppLogger _log = loggerFor('zeta.agent.grok_models_cli');
...
class GrokModelsCli {
  ...
  Future<AgentModelList> listModels(AgentProviderConfig config) async {
    ...
    if (resolved == null) {
      ...
      return AgentModelList(models: const <AgentModelInfo>[]);
    }

    try {
      final result = await processRunner(
        resolved.executable,
        resolved.processCommandFor(const <String>['models']).arguments,
