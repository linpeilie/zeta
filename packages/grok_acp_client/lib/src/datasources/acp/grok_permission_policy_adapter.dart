...
final AppLogger _log = loggerFor('zeta.agent.grok_permission_policy');

/// 发送 live 权限通知（由 provider 注入，best-effort）。
typedef GrokPermissionLiveNotifier = void Function(
  String method,
  ...
  Future<AgentPermissionApplyResult> applyPermissionSelection(
    ...
    if (!isInitialized() || isDisposed()) {
      ...
      return AgentPermissionApplyResult(
        ...
        warningCode: isDisposed()
            ? AgentPermissionWarningCode.appliesNextSession
            : null,
      );
    }

    const method = GrokPermissionModeCodec.yoloModeChangedMethod;
