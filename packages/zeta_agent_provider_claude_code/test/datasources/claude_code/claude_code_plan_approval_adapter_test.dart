    ...
    test(
      ...
      () {
        final frames = _loadFixture(
          providerTestFiles.path(
            'test/datasources/claude_code/'
            'fixtures/exit_plan_mode.jsonl',
          ),
        );
...
ClaudeCodePlanApprovalAdapter _adapterWithObservedPlan({
  ...
  final adapter = ClaudeCodePlanApprovalAdapter();
  adapter.beginTurn(sessionId: 'session-plan-1', turnId: 'turn-plan-1');
  adapter.recordAssistantText(
    sessionId: 'session-plan-1',
    turnId: 'turn-plan-1',
    text: 'Fallback plan text',
  );
  adapter.recordExitPlanToolUse(
    toolUseId: 'toolu-plan-1',
    input: input,
    sessionId: 'session-plan-1',
    turnId: 'turn-plan-1',
