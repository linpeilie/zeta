...
codex 的独立纯 Dart Provider 插件。生产入口是 `zeta_agent_provider_codex.dart`；宿主测试的实现类型只从独立 `zeta_agent_provider_codex_testing.dart` 入口取得。
...
也可在仓库根目录运行 `flutter test packages/zeta_agent_provider_codex`，资源按 package config 定位，不依赖当前目录。
