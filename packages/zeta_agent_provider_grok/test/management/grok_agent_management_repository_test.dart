...
import 'package:zeta_agent_provider_grok/src/management/grok_agent_management_repository.dart';
  ...
  group('GrokAgentManagementRepository', () {
    ...
    test('detects installed CLI and marks update available', () async {
      ...
      final registry = AgentProviderRuntimeRegistry(
        providerFactory: _DetectionProviderFactory(),
      );
...
class _ProbeFakeProvider implements AgentRuntimePort, AgentConversationPort {
...
// 管理检测仅借用初始化与空模型目录，保留原工作台 fake 在此路径的行为。
final class _DetectionProviderFactory with TestAgentProviderBundleFactory {
  @override
  Object create(AgentProviderConfig config) => _DetectionProvider(config);
}

final class _DetectionProvider extends _ProbeFakeProvider
    implements AgentModelCatalogPort {
  _DetectionProvider(super.config);
  @override
  Future<AgentModelList> listModels({
    int limit = 20,
    bool includeHidden = false,
    bool forceRefresh = false,
  }) async => const AgentModelList(models: <AgentModelInfo>[]);
}
