...
class ClaudeCodeSessionHistoryReader {
  ...
  }) : assert(sampleWindowBytes > 0),
       assert(sampleLineLimit > 0),
  ...
  Future<AgentThreadPage> listThreads({
    ...
    final filtered = searchTerm == null || searchTerm.isEmpty
        ...
        : summaries
              .where(
                (summary) =>
                    summary.displayName.toLowerCase().contains(searchTerm) ||
...
final class _MutableHistoryTurn {
  ...
  void upsertTool(AgentToolCall incoming, {required DateTime? occurredAt}) {
    ...
    final title =
        ...
        ? previousTool.title
        : incoming.title;
    final kind = incoming.kind == AgentToolKind.other && previousTool != null
        ? previousTool.kind
        : incoming.kind;
    ...
    _upsert(
      AgentHistoryToolEntry(
        toolCall: incoming.copyWith(
          ...
          locations: incoming.locations.isEmpty
              ? previousTool?.locations
              : incoming.locations,
          startedAt: previousTool?.startedAt ?? occurredAt,
          completedAt: terminalAt ?? previousTool?.completedAt,
          rawInput: incoming.rawInput.isEmpty
...
String _historyUserEntryId({
  ...
  return 'cc-history-user:${_stableToken('$sessionId\u0000$turnId\u0000$sourceId')}';
}
...
String _defaultHistoryTurnId(
  ...
  return 'cc-history-turn:${_stableToken('$sessionId\u0000$sourceTurnId\u0000$ordinal')}';
}
