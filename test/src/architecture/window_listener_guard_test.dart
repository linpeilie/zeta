...
/// 平台 `WindowListener` 只允许出现在窗口表面与 native host 的关窗拦截。
///
/// `MainApp` / `IdeHome` / feature Widget 禁止再 mixin：窗口事件由
/// `ZetaWindowSurfaceNotifier` 翻译成快照，消费方只读状态。
void main() {
  test('lib 里 with WindowListener 只出现在窗口表面与 native host', () {
    const allowed = <String>{
      'lib/src/app/window/zeta_window_surface.dart',
      'lib/src/app/window/zeta_window_host.dart',
    };
    ...
    expect(
      ...
      reason:
          ...
          'zeta_window_host.dart。请改读 zetaWindowSurfaceProvider：\n'
          '${offenders.join('\n')}',
    );
  });
}
