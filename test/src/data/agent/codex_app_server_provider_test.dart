...
import 'package:zeta/src/data/agent/codex_app_server_provider.dart';
...
void main() {
  group('CodexAppServerAgentProvider', () {
    ...
    test('reads thread history with turns and maps items', () async {
      ...
      expect(history.turnsById.keys, <String>['turn-1']);
      expect(history.currentTurn?.id, 'turn-1');

      final turn = history.turnsById['turn-1']!;
      ...
      final userMessage = history.entries[0] as AgentHistoryMessageEntry;
      expect(userMessage.role, AgentMessageRole.user);
      expect(userMessage.text, 'Hello Agent');

      final agentMessage = history.entries[1] as AgentHistoryMessageEntry;
      ...
      final fileChange = history.entries[4] as AgentHistoryToolEntry;
    ...
    test('prefers local session jsonl history when available', () async {
      ...
      expect(history.turnsById.keys, <String>['turn-local']);
      expect(history.currentTurn?.id, 'turn-local');

      final turn = history.turnsById['turn-local']!;
      ...
      final command = history.entries[2] as AgentHistoryToolEntry;
      expect(command.toolCall.title, 'flutter test');
      expect(command.toolCall.kind, AgentToolKind.execute);
      expect(command.toolCall.content, 'flutter test');
      expect(command.toolCall.locations, <String>['/repo']);

      final patch = history.entries[3] as AgentHistoryToolEntry;
    ...
    test(
      ...
      () async {
        ...
        expect(history.entries, hasLength(4));
        expect(
          history.entries.whereType<AgentHistoryEventEntry>().map(
            (entry) => entry.kind,
          ),
        ...
        final permission = history.entries[0] as AgentHistoryEventEntry;
        ...
        final toolSearch = history.entries[1] as AgentHistoryEventEntry;
        expect(toolSearch.title, 'Tool search');
        expect(toolSearch.content, 'rip_grep_packages\nlimit=8');

        final warning = history.entries[2] as AgentHistoryEventEntry;
        expect(warning.title, 'Config Warning');
        expect(warning.description, 'History may be incomplete');

        final webSearch = history.entries[3] as AgentHistoryEventEntry;
    ...
    test(
      ...
      () async {
        ...
        final entry = history.entries.single as AgentHistoryEventEntry;
    ...
    test(
      ...
      () async {
        ...
        final entry = history.entries.single as AgentHistoryToolEntry;
    ...
    test('skips bad lines and keeps remaining local history', () async {
      ...
      expect(
        (history.entries[0] as AgentHistoryToolEntry).toolCall.title,
        'dart format .',
      );
      expect(
        (history.entries[1] as AgentHistoryMessageEntry).text,
        'Still recover this',
      );
