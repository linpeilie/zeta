...
void main() {
  ...
  testWidgets(
    'keeps live Chinese markdown at conversation width and clears running status',
    (tester) async {
      const paragraph = '我来梳理 UI 设计令牌与可配置参数的来源，不只看颜色和尺寸文件。';
      final session = MemorySessionStore();
      final provider = FakeAgentProvider(
        completeTurns: false,
        responseText: '',
      ...
      final markdownFinder = find.descendant(
        of: liveSectionFinder,
        matching: find.byType(MarkdownWidget),
      );
      expect(markdownFinder, findsOneWidget);
      expect(
        tester.widget<MarkdownWidget>(markdownFinder).controller,
        isNotNull,
      );
      ...
      expect(find.text('42 次思考'), findsOneWidget);
      expect(markdownFinder, findsOneWidget);
      expect(tester.widget<MarkdownWidget>(markdownFinder).controller, isNull);
      expect(tester.getSize(markdownFinder).width, greaterThan(500));
      expect(
        find.textContaining(paragraph, findRichText: true),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey<String>('agent-live-activity-status')),
        findsOneWidget,
      );
