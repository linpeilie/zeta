...
void main() {
  group('Phase 2 切片验收', () {
    testWidgets('同一应用里两个会话的切片完全隔离', (tester) async {
      ...
      final firstStore = connectedConversationTestOwner(
        regions: first,
        commands: first,
      );
      final secondStore = connectedConversationTestOwner(
        regions: second,
        commands: second,
      );
      addTearDown(firstStore.closeForEntryRelease);
      addTearDown(secondStore.closeForEntryRelease);
      ...
      await tester.pumpWidget(
        _TwoPaneApp(
          first: first,
          second: second,
          stores: <AgentConversationBindingKey, AgentConversationSliceNotifier>{
      ...
      expect(
        secondStore.current.header.title,
        isNot('只属于会话一'),
    ...
    testWidgets('dispose 其中一个会话，另一个仍然工作', (tester) async {
      ...
      firstStore.closeForEntryRelease();
      await pumpAgentPaneUi(tester);
...
class _TwoPaneApp extends StatelessWidget {
  ...
  final Map<AgentConversationBindingKey, AgentConversationSliceNotifier> stores;
