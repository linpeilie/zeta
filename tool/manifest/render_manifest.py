...
w("| 最终迁移基线 SHA | `b5c2f3e8a9ac544e9832866e86ff633661c46053` |")
w("| 工作区状态 | tracked clean（唯一未跟踪项：`.workflow/feature/2026-08-18-PC端构建与版本检查/`，明确排除） |")
w(f"| Git 跟踪文件总数 | **{len(rows):,}** |")
...
w("")
w("> [!IMPORTANT]")
w("> **这是 Step 0 清退完成后的最终迁移基线。** 清退提交中的 5 个 Cursor 源码、测试与 fixture")
w("> 已从 Git 索引消失，因此不再出现在本清单中。迁移执行期不得改用其他旧仓库提交。")
w("")
w("## 2. 动作定义")
w("")
w("| 动作 | 含义 | 验证方式 |")
...
# ---------------------------------------------------------------- lib
w(f"## 6. `lib/` 逐文件（{len(sel(lambda p: p.startswith('lib/')))}）")
w("")
