...
w("| Analysis baseline SHA | `bfd42412c9c3a0b39bb93598f93f9e5eca471236` |")
w("| Working tree | clean (only untracked path: `.workflow/feature/2026-08-18-PC端构建与版本检查/`) |")
w("| Git-tracked files | **1,512** |")
w("| Manifest generated | 2026-08-19 |")
w("| Target repository | `D:\\Development\\Workspace\\vgv\\zeta`, version `1.0.0+1` |")
w("")
w("> [!IMPORTANT]")
w("> **This manifest is based on analysis baseline `bfd4241`, which is not the final migration baseline.**")
w("> [Step 0](./migration_tasks.md) (Cursor retirement) changes the old repository. When executing")
w("> [step 1](./migration_tasks.md) you must:")
w("> ")
w("> 1. Re-run this manifest's generator to obtain the post-retirement SHA and file count;")
w("> 2. Record the final Flutter/Dart versions and the `pubspec.lock` hash;")
w("> 3. Confirm the Cursor-related files marked `delete` are gone from the old repo (at that point they")
w(">    no longer appear in this manifest at all).")
w("")
