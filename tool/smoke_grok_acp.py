...
docs/zh/protocols/grok_acp_protocol.md §14.1 要求：AC1（会话级独立进程）
...
冒烟约束：临时隔离 workspace（每个 session 一个空目录，不含真实项目文件，交互只读）、
minimal 权限（默认 ask 模式；任何 session/request_permission 或其它
server->client 请求一律拒绝，保持非破坏性）、prompt 极简且不触发工具调用。

记录约束：只记录"哪个阶段成功/失败"及脱敏协议字段名，不记录 prompt/回复原文、
字段值、原始 payload、sessionId、stderr 原文或凭据。
"""
...
class Peer:
    ...
    extension_prefixes: set[str] = field(default_factory=set)
    _next_id: int = 1
            ...
            with self._cv:
                ...
                if isinstance(method, str):
                    if method.startswith("_x.ai/"):
                        self.extension_prefixes.add("_x.ai/")
                    elif method.startswith("x.ai/"):
                        self.extension_prefixes.add("x.ai/")
...
def run_session(
    ...
    try:
        ...
        auth_fields = sorted(
            {
                str(key)
                for item in (auth_methods if isinstance(auth_methods, list) else [])
                for key in (item.keys() if isinstance(item, dict) else ())
            }
        )
        billing_config_fields: list[str] = []
        if label == "session-a":
            try:
                billing = peer.request("_x.ai/billing", timeout=30)
                config = dig(billing, "result", "config")
                if isinstance(config, dict):
                    billing_config_fields = sorted(str(key) for key in config)
            except TimeoutError:
                pass
        ...
        results[label] = {
            ...
            "billingConfigFields": billing_config_fields,
            "extensionPrefixes": sorted(peer.extension_prefixes),
        }
    except TimeoutError:
...
def run_recovery(
    ...
    try:
        ...
        return {"ok": False, "stage": f"{stage}/timeout"}
    except Exception as exc:  # noqa: BLE001 - 冒烟脚本，记录类型即可
        return {
            "ok": False,
...
def main() -> int:
    ...
    try:
        ...
        if isinstance(session_a_id, str) and session_a_id:
            # Grok may persist session metadata asynchronously after the prompt
            # terminal. Give the closed source process a bounded flush window
            # before a new process attempts session/load.
            time.sleep(2)
    ...
    sample = results.get("session-a", {})
    check(
        checks,
        "protocol metadata sampled without payload values",
        bool(sample.get("authFields")),
        "authFields=" + ",".join(sample.get("authFields", []))
        + "; extensionPrefixes="
        + ",".join(sample.get("extensionPrefixes", []))
        + "; billingConfigFields="
        + ",".join(sample.get("billingConfigFields", [])),
    )
